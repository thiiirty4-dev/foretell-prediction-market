...
import { getProductUser, recordMarketCreated } from "@/db/product";
...
export async function POST(request: Request) {
  try {
    ...
    const market = await insertMarket({ title, description, category, closesAt });
    await recordMarketCreated(user.id, market.id);
