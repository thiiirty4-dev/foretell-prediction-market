...
export async function POST(request: Request) {
  try { const blocked = await protectMutation(request, "market-alert", 30, 60_000); if (blocked) return blocked; const user = await getProductUser(request); if (!user) return NextResponse.json({ error: "Log in to manage price alerts" }, { status: 401 }); const body = await request.json() as Record<string, unknown>; const action = body.action;
